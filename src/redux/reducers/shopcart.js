...
const shopCart = (state=initialState, action) => {
    let newState = {...state}; 
    const index = newState.shopCart.findIndex(el=> el._id === action.payload._id)
    if(json) {
        newState = json; 
    }
    switch(action.type) {
        case ADD_ITEM_CAR:
            if(index !== -1) {
                newState.shopCart[index].quantity =  newState.shopCart[index].quantity + 1
            } else {
        ...
        case REMOVE_ITEM_CAR:
            if(index !== -1) {
                if(newState.shopCart[index].quantity > 1) {
                    ...
                    newState.shopCart.filter(el=> el._id !== action.payload._id)
                }
            } 
        break

        case REMOVE_ALL_ITEM_CAR:
            newState.shopCart = newState.shopCart.filter(el=> el._id !== action.payload._id);
        break
